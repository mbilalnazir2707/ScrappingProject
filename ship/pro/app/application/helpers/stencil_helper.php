...
{
	function add_front_css($css = NULL)
	{
		if (is_null($css))
		{
			return FALSE;
		...
		{
			$file_type = (preg_match('/\.css$/i', $css) ? NULL : '.css');
			$url = (!preg_match('#^www|^http|^//#', $css)) ? base_url('assets/front/css/'.$css.$file_type) : $css;
			return '<link rel="stylesheet" href="'.$url.'">'."\n";
		}
		else
		{
			...
			foreach ($css as $item)
			{
				...
				$file_type = (preg_match('/\.css$/i', $item) ? NULL : '.css');
				$url = (!preg_match('#^www|^http|^//#', $item)) ? base_url('assets/front/css/'.$item.$file_type) : $item;
				$items[] = '<link rel="stylesheet" href="'.$url.'">'."\n".$tab;
				$i++;
			}
			return implode('', $items);
...
if (!function_exists('add_front_js'))
{
	function add_front_js($js = NULL)
	{
		...
		{
			$file_type = (preg_match('/\.js$/i', $js) ? NULL : '.js');
			$url = (!preg_match('#^www|^http|^//#', $js)) ? base_url('assets/front/js/'.$js.$file_type) : $js;
			return '<script src="'.$url.'"></script>'."\n";
		}
		else
		{
			...
			foreach ($js as $item)
			{
				...
				$file_type = (preg_match('/\.js$/i', $item) ? NULL : '.js');
				$url = (!preg_match('#^www|^http|^//#', $item)) ? base_url('assets/front/js/'.$item.$file_type) : $item;
				$items[] = '<script src="'.$url.'"></script>'."\n".$tab;
				$i++;
			}
			return implode('', $items);
