$(function() {
	$('#setting_signature').signature({
		 change: function(event, ui) { 
			...
			signature_svn = signature_svn.replace('<svg xmlns="http://www.w3.org/2000/svg" width="15cm" height="15cm">','<svg xmlns="http://www.w3.org/2000/svg" width="200" height="87">');
	...
	$('#setting_clear').click(function() {
		...
		$('#signature_svn_txt').val('');
		$('#svn_signature_code').html(old_signature);
		
		// $('#sig_draw_div').removeClass('d-none');
		// $('#sig_preview_div').addClass('d-none');

		// $('#patient-signature-save').removeClass('d-none');
